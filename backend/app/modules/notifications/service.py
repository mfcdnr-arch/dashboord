...
EVENT_LABELS = {
    ...
    # Дыра ВНУТРИ ряда — не то же самое, что «последний отчёт не пришёл»:
    # ряд продолжился, и без отдельного события пропуск не заметил бы никто.
    "data.gap": "В ряду отчётов пропуск",
    ...
    # Не «после автопочинки»: сигнал уходит по СОСТОЯНИЮ системы, а чинить
    # приложение умеет лишь малую часть причин (бакет MinIO, связь с Redis).
    "system.degraded": "Система в плохом состоянии",
    ...
    # Одно уведомление на ПЕРВЫЙ просмотр: до него обращение выглядит так же,
    # как в момент отправки, то есть как будто ушло в пустоту.
    "appeal.seen": "Ваше обращение просмотрено",
    # Одно уведомление на авто-выпуск: данные ушли на дашборды без нажатия
    # кнопки, и узнать об этом иначе можно было только заметив бейдж в папке.
    "data.auto_released": "Данные выпущены автоматически",
    # Воркер тихо останавливает весь конвейер, а поднять его может только хост.
    # Молчаливый перезапуск скрыл бы, что воркер падает регулярно.
    "system.worker_restarted": "Фоновый воркер перезапущен автоматически",
}
