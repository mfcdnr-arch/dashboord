...
async def quality_warnings(conn, org_id, *, code: str, period, rows: List[List[str]],
    ...
    try:
        ...
        #
        # 🔴 Но и МОЛЧАТЬ нельзя (20.09.2026). Раньше здесь возвращался пустой
        # список, а пустой список означает «замечаний нет»: сбой ОДНОГО из
        # девяти правил молча отключал все разом, модератор видел «проверок
        # нет — значит всё хорошо», а авто-выпуск (условие `if warnings:
        # return`) считал данные проверенными и отправлял их на дашборд.
        # Это и была та «цена страховки», названная в журнале 02.09.
        #
        # Поэтому возвращаем ЗАМЕЧАНИЕ о самом сбое: модератор видит, что
        # проверок не было, а авто-выпуск останавливается сам — список не
        # пуст. Отдельного признака заводить не нужно, и три места, которые
        # читают эти замечания, лечатся одной правкой.
        log.warning("Проверки качества не отработали (%s) — выпуск продолжается", e)
        return [{
            "code": "quality_checks_failed",
            "count": 1,
            "message": ("Проверки качества не отработали из-за внутренней ошибки — данные "
                        "НЕ проверены ни на арифметику внутри файла, ни на совпадение с "
                        "прошлым отчётом. Выпустить можно, но сверьте цифры сами и "
                        f"сообщите администратору: {e}"),
        }]
