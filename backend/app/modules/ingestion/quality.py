...
# Насколько выпуск должен «похудеть» относительно предыдущего, чтобы это
# перестало быть колебанием загрузки и стало поводом посмотреть.
#
# Порог подобран ЗАМЕРОМ по 53 парам выпусков ежедневного отчёта РЦО, а не на
# глаз: рабочие дни дают 100 % заполненных клеток от предыдущего, короткие дни
# (суббота) — около 50 %, а лист-заготовка ещё не заполненного дня — 0,92 %.
# Второе снизу значение по всей истории — 45,79 %. Порог 10 % лежит между ними
# с пятикратным запасом в обе стороны.
#
# Нижняя граница по размеру предыдущего выпуска нужна, чтобы правило молчало на
# мелких формах: у выпуска из десятка клеток доля скачет от одной ячейки.
ALMOST_EMPTY_RATIO = 0.10
ALMOST_EMPTY_MIN_PREV = 50

MIN_FIELDS_MULTIPLIED = 4
MULTIPLIED_BAND = 0.001

# «Форма заполнена, но работы в ней нет»: какая доля значений должна быть
# ненулевой и с какого размера формы вопрос вообще имеет смысл.
#
# Порог подобран ЗАМЕРОМ по всем выпускам дев-стенда, а не на глаз. Ежедневный
# отчёт РЦО: 28.08.2026 — 42 ненулевых значения из 20 212 заполненных (0,21 %),
# тогда как накануне и назавтра 1 064 и 1 458 (5,3 и 7,2 %), а короткая суббота
# 29.08 — 573 из 10 874 (5,3 %). Ниже 5,3 % не опускается НИ ОДИН из 54 дней.
#
# Верхнюю границу задаёт не РЦО, а самая РАЗРЕЖЕННАЯ живая форма стенда:
# «Статистика услуг — Росимущество» — 5 ненулевых из 248 (2,02 %) во всех трёх
# отчётах. Это норма, а не сбой: ведомство оказывает услуги в единицах
# отделений, и сработай правило на нём — его приучились бы пролистывать.
# Порог 0,6 % лежит между 0,21 % и 2,02 % с трёхкратным запасом в обе стороны:
# он переживёт и потерю Росимуществом трёх активных отделений из пяти (0,81 %),
# и день, втрое более «живой», чем 28.08.
#
# Нижняя граница по числу клеток проводит ещё и границу с `_check_almost_empty`:
# лист-заготовка 01.09 (186 клеток) — это «клеток почти нет», и о нём говорит
# ТА проверка; здесь речь о форме, заполненной целиком. Одно событие — одно
# замечание. Заодно граница отсекает мелкие формы стенда (13, 30, 32 и 124
# клетки), где одна ячейка меняет долю на целые проценты.
NONZERO_SHARE_MIN = 0.006
NONZERO_MIN_CELLS = 200
...
def check_internal(current: Dict[Key, float], names: Dict[str, str]) -> List[dict]:
    ...
    return [w for w in (_check_total_row(current, names),
                        _check_total_column(current, names),
                        _check_weekly_over_total(current, names),
                        _check_fact_over_plan(current, names),
                        _check_all_zeros(current)) if w]
...
def _check_total_column(current: Dict[Key, float], names: Dict[str, str]) -> Optional[dict]:
    """Итоговая ГРАФА против суммы составляющих граф — зеркало `_check_total_row`.

    Та сверяет итоговую СТРОКУ с суммой строк; здесь наоборот. Второй такой
    сверки в системе не было, а форма РЦО состоит из неё целиком: 326 граф вида
    «Ведомство · Услуга · Принято, ед.» и рядом «ИТОГО · Принято, ед.».

    Показатель определяется ХВОСТОМ имени после последнего « · »: у РЦО это
    «Принято, ед.» / «Выдано, ед.» / «Отказ, ед.», и складывать можно только
    внутри одного хвоста. Иначе «Принято» и «Выдано» попадут в одну сумму, а это
    две стадии ОДНОГО обращения, то есть двойной счёт. Если форма своего
    устройства не называет (в именах нет « · »), каждая графа оказывается в
    своей группе и правило молчит: угадывать, какие графы складываются, нельзя —
    ошибка здесь дала бы уверенное с виду, но выдуманное расхождение.

    🔴 Графу-итог ищем ПО ИМЕНИ, хотя в проекте принято искать её арифметикой —
    равенством сумме остальных (так делает виджет «Показатели списком»). Здесь
    арифметика не годится ПРИНЦИПИАЛЬНО: правило существует затем, чтобы поймать
    графу-итог, которая сумме НЕ равна, а поиск по равенству нашёл бы только уже
    сошедшиеся — то есть правило молчало бы всегда. Выражение берём то же, что у
    `_check_total_row`: двух понятий об «итоге» в системе быть не должно. Ищем в
    ПРЕФИКСЕ имени, а не во всём: иначе услуга со словом «итог» в названии
    объявила бы итогом сама себя.

    Допуск общий с `_check_total_row`, и замер это подтверждает. По 54 выпускам
    РЦО из 9 243 пар (день × отделение × показатель) 9 240 сходятся с
    расхождением не больше 1, а три оставшиеся расходятся на 25, 69 и 124 — это
    одно отделение (Волноваха), где «ИТОГО · Принято» равен 1 при сумме услуг
    125. Разрыв полный: округление правило не тревожит, настоящую ошибку видит.
    """
    from ..dashboards._aggregate import is_share  # локально: иначе цикл импорта
    ...
    for code in sorted({k[1] for k in current}):
        name = names.get(code, code)
        # Доли и проценты не складываются (правило одно на систему —
        # `dashboards._aggregate`): их итог — среднее, а не сумма.
        if is_share(name):
            continue
        groups.setdefault(_head_tail(name)[1], []).append(code)

    rows = sorted({k[0] for k in current})
    bad: List[str] = []
    for tail, codes in sorted(groups.items()):
        totals = [c for c in codes if _TOTAL_ROW_RE.search(_head_tail(names.get(c, c))[0])]
        # Ровно одна графа-итог: две «итоговых» подписи в группе означают, что
        # устройство формы мы не понимаем, и молчать честнее, чем гадать.
        if len(totals) != 1:
            continue
        total_code = totals[0]
        parts = [c for c in codes if c != total_code]
        if len(parts) < 2:
            continue
        for row in rows:
            total = current.get((row, total_code))
            if total is None:
                continue
            vals = [current[(row, c)] for c in parts if (row, c) in current]
            # Та же оговорка, что у сверки строк: одна составляющая — это не
            # сумма, а сама эта графа, и правило выродилось бы в проверку
            # «графа равна итогу».
            if len(vals) < 2:
                continue
            got = sum(vals)
            diff = got - total
            if abs(diff) > max(TOTAL_TOLERANCE_ABS, abs(total) * TOTAL_TOLERANCE_REL):
                bad.append(f"«{tail}» в строке «{row}»: сумма по {len(vals)} графам {_fmt(got)}, "
                           f"в графе «{names.get(total_code, total_code)}» {_fmt(total)} "
                           f"(расхождение {_fmt(abs(diff))})")
    if not bad:
        return None
    return {"code": "total_column_mismatch", "count": len(bad),
            "message": ("Итоговая графа не сходится с суммой составляющих — проверьте, все ли "
                        "графы на месте и не потерялась ли услуга: " + "; ".join(bad[:5]))}


def _check_all_zeros(current: Dict[Key, float]) -> Optional[dict]:
    """Форма заполнена целиком, а работы в ней нет.

    🔴 Найдено на данных заказчика и НЕ ловилось ничем. 28.08.2026 у ежедневного
    отчёта РЦО заполнено 20 212 клеток — ровно столько же, сколько накануне, —
    но ненулевых из них 42, по услугам не принято ни одного обращения, а в графе
    «ИТОГО · Принято» стоит 1.

    Соседняя проверка `_check_almost_empty` такой день пропускает ПО УСТРОЙСТВУ:
    она считает ЗАПОЛНЕННЫЕ клетки, а ноль — заполненная клетка. Там речь про
    «клеток почти нет», здесь — про «клетки есть, чисел в них нет». Это разные
    события, и замечание о каждом своё.

    Цена пропуска высокая: виджеты читают последний выпуск, поэтому весь дашборд
    показывает нули, и человек видит не «день пустой», а «система сломалась».

    Правило не блокирует выпуск — как и все остальные: настоящий нерабочий день
    существует, и решает человек.
    """
    if len(current) < NONZERO_MIN_CELLS:
        return None
    nonzero = sum(1 for v in current.values() if v)
    share = nonzero / len(current)
    if share >= NONZERO_SHARE_MIN:
        return None
    pct = f"{share * 100:.2f}".replace(".", ",")
    return {
        "code": "all_zeros", "count": nonzero,
        "message": (f"Форма заполнена ({len(current)} значений), но ненулевых из них всего "
                    f"{nonzero} ({pct} %) — похоже, за этот день данные не внесены. "
                    "Выпуск возможен, но виджеты читают последний отчёт, и дашборд покажет нули "
                    "по всем показателям."),
    }
...
def _check_almost_empty(current: Dict[Key, float], previous: Dict[Key, float]) -> Optional[dict]:
    """Выпуск почти пустой рядом с предыдущим.

    🔴 Найдено боевой загрузкой РЦО: последним листом книги оказалась ЗАГОТОВКА
    ещё не заполненного дня — 186 чисел вместо двадцати тысяч, из них 185
    нулей. Данные при этом верные: день просто не наступил. Но виджет читает
    ПОСЛЕДНИЙ выпуск, поэтому дашборд открывался пустым, и человек видел нули
    там, где ждал цифры, — то есть решал, что сломана система.

    Пустой выпуск не ошибка и не блокируется: бывает и нерабочий день. Правило
    только называет вещи своими именами, как и остальные проверки.

    Считаем по числу ЗАПОЛНЕННЫХ числовых клеток, а не по сумме значений: день
    с настоящими нулями по всем услугам — это заполненный день, и путать его с
    незаполненным нельзя.
    """
    if len(previous) < ALMOST_EMPTY_MIN_PREV:
        return None
    ratio = len(current) / len(previous)
    if ratio > ALMOST_EMPTY_RATIO:
        return None
    pct = f"{ratio * 100:.1f}".replace(".", ",")
    return {
        "code": "almost_empty", "count": len(current),
        "message": (f"В отчёте заполнено {len(current)} значений против {len(previous)} в прошлом "
                    f"({pct} % от прежнего) — похоже, форма ещё не заполнена. "
                    "Выпуск возможен, но виджеты читают последний отчёт, и дашборд покажет "
                    "пустые цифры."),
    }
...
def compare_with_previous(
    ...
    for row in sorted(rows_now):
        ...
        if not cells or not prev_cells:
            continue
        # 🔴 Строка, где и там и там ОДНИ НУЛИ, совпадением не считается.
        # Повторившийся ноль не свидетельствует о переносе цифр: он означает,
        # что отделение за этот день не работало, — а в ежедневном отчёте на
        # 62 отделения таких строк десятки. Найдено проверкой истории РЦО
        # 22.09.2026: правило срабатывало на 63 отчётах из 201, и почти все
        # срабатывания были про пустые отделения. Сигнал, который поднимают
        # зря, перестают читать, и тогда он не сработает в настоящий раз.
        #
        # Случай «весь отчёт из нулей» при этом НЕ теряется: его ловит
        # собственное правило `all_zeros`, и оно говорит о нём прямо.
        if not any(cells.values()) and not any(prev_cells.values()):
            continue
    ...
    # ── 6. Выпуск почти пустой ──────────────────────────────────────────────
    empty = _check_almost_empty(current, previous)
    if empty:
