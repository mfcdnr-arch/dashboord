...
# Кавычки всех начертаний, которые встречаются в госформах: ёлочки, лапки,
# типографские одиночные. Приводим к простой двойной.
_QUOTES_RE = re.compile(r"[«»“”„‟”\u2018\u2019]")


def clean_row_label(value) -> Optional[str]:
    """Та же нормализация — для СТРОКИ-МЕТКИ (адрес отделения и т.п.), не
    только заголовка столбца.

    Реальный случай (ДНР_статистика): у части ведомств адрес отделения нёс
    лишний пробел или перенос по краям ячейки, которого не было у соседнего
    ведомства с ТЕМ ЖЕ офисом, — раздел «Статистика услуг» считал их РАЗНЫМИ
    отделениями (63 → 71 в своде), потому что row_label писался как есть, без
    очистки. Строки без пробельных огрехов проходят как есть.

    Второй случай той же природы (файлы РЦО, 02.09.2026): в одном и том же
    файле кавычки скачут — «МФЦ ДНР» ёлочками в одной строке и "МФЦ ДНР"
    прямыми в соседней. Для человека это одно отделение, для ключа строки —
    два разных, и своды разъезжаются точно так же. Приводим все виды кавычек
    к прямым: это НЕ потеря смысла, потому что кавычки здесь оформление
    названия, а не его часть.
    """
    if value is None:
        return None
    return _clean(_QUOTES_RE.sub('"', str(value)))
