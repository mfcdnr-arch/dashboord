...
# Предел поля `name` в API создания метрики (MetricIn.name).
METRIC_NAME_MAX = 200


def _fit_name(name: str) -> str:
    """Уложить название предложения в предел поля.

    Названия строятся из имён граф госформы, а они длинные: на реальной форме
    заказчика «А — доля от «Б» (нарастающим итогом), %» дало 204 символа при
    пределе 200, и создание метрики отклонялось с 422 — человек видел голое
    «Ошибка (422)» и не понимал, что не так.

    Режем СЕРЕДИНУ: у таких названий смысл как раз по краям — что от чего
    считается и в каком разрезе. Обрезка с конца съела бы разрез и сделала
    названия неразличимыми (та же причина, по которой подписи виджетов
    сокращаются серединой).
    """
    if len(name) <= METRIC_NAME_MAX:
        return name
    keep = METRIC_NAME_MAX - 1  # один символ под многоточие
    head = keep * 55 // 100
    return name[:head].rstrip() + "…" + name[-(keep - head):].lstrip()
...
def _assign_codes(specs: List[dict], existing_codes: set) -> None:
    ...
    for spec in specs:
        # Имя укладываем ДО вычисления кода, чтобы код соответствовал тому,
        # что человек видит в списке.
        spec["name"] = _fit_name(spec["name"])
