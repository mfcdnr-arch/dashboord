...
TEMPLATES: List[Dict[str, Any]] = [
    ...
    {
        # Отличается от «прироста к прошлому периоду» точкой отсчёта: там шаг
        # назад, здесь — НАЧАЛО наблюдений. На внедрении сервиса это главный
        # вопрос руководителя: «на сколько выросли с начала», а не «за неделю».
        "code": "vs_first_pct", "group": "Динамика", "name": "Рост от начального значения, %", "unit": "%",
        "description": "Последнее значение в процентах к самому первому периоду данных: "
                       "140 % — рост в 1,4 раза с начала наблюдений.",
        "example": "Насколько выросли обращения с начала внедрения сервиса",
        "min_periods": 2,
        "inputs": [{"key": "a", "kind": "field", "agg": "SUM", "label": "Столбец"}],
        "formula": "PERIOD_COMPARE({a}, 'first', mode='pct')",
    },
    {
        "code": "vs_first_delta", "group": "Динамика", "name": "Прирост от начального значения", "unit": None,
        "description": "На сколько единиц значение выросло с самого первого периода данных.",
        "example": "Насколько больше уведомлений доставляется, чем в первую неделю",
        "min_periods": 2,
        "inputs": [{"key": "a", "kind": "field", "agg": "SUM", "label": "Столбец"}],
        "formula": "PERIOD_COMPARE({a}, 'first')",
    },
    {
