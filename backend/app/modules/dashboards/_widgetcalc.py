...
# Столбиков на графике «по строкам». У ежедневного отчёта РЦО 63 отделения, и
# график превращался в сплошной мазок: столбики шириной в волос, а 63 повёрнутые
# подписи по 300px наезжают друг на друга (найдено осмотром дашборда заказчика).
#
# Порог 12 — по ЗАМЕРУ, а не на глаз. Сначала я поставил 16 и обрезкой не
# обошёлся: на карточке в половину ряда (642px) слот категории — 38px, а подпись
# отделения при 11px занимает 138px, поэтому 16 подписей давали 27 наложений.
# Ответ оказался не в числе столбиков, а в РАЗВОРОТЕ графика: имена отделений
# длинные, и на горизонтальных полосах они читаются построчно, без поворота.
...
def _trim_bars(res: dict) -> None:
    """Оставить читаемое число столбиков и СКАЗАТЬ, сколько строк не показано.

    То же правило, что у «Сравнения показателей», «Ранжированного списка» и
    «Показателей списком»: молчаливой обрезки быть не должно — виджет, тихо
    показавший часть данных, читается как показавший все.

    Отбираем по ОБЪЁМУ (модуль значения: минус объём не уменьшает), а ПОКАЗЫВАЕМ
    в порядке формы — перетасовка при каждом открытии мешала бы сверять виджет с
    файлом. Полный список остаётся в таблице и в рейтинге на той же странице.
    """
    cats = res.get("categories") or []
    vals = res.get("values") or []
    if len(cats) <= MAX_ROW_BARS:
        return
    idx = sorted(range(len(cats)),
                 key=lambda i: abs(vals[i]) if i < len(vals) and vals[i] is not None else 0.0,
                 reverse=True)[:MAX_ROW_BARS]
    idx.sort()
    res["categories"] = [cats[i] for i in idx]
    res["values"] = [vals[i] for i in idx]
    res["hidden_rows"] = len(cats) - len(idx)
    res["total_rows"] = len(cats)
...
async def _compute_widget_inner(conn, org_id, t: str, name: str, cfg: dict,
    ...
    if t == "field_list":
        ...
        # Группировка — по разделителю из настройки; не задан — определяем его
        # по самим именам формы (`pick_separator`, тем же правилом, что и
        # лестница). Зашивать разбор нельзя: у РЦО имена вида «Ведомство ·
        # Услуга · Показатель», у «Статистики услуг» — «Услуга 5: Принято».
        # Раньше разделитель приходилось вписывать руками в каждый виджет, и
        # на форме со вторым разделителем список приходил вовсе без групп —
        # то есть вид, заведённый ради вопроса «какие услуги идут», отвечал на
        # него хуже всего там, где услуг больше всего.
        sep = cfg.get("group_sep") or pick_separator([x["name"] for x in items])
        for x in items:
        ...
        for x in sums:
            rest = grand - x["value"]
            # Пустая графа итогом быть не может. Найдено замером: 28.08 у РЦО все
            # 143 услуги «Принято» по нулям, а «ИТОГО · Принято» = 1 — нижний
            # порог допуска (1,0) совпадал тогда с ЛЮБЫМ нулём, и итогом
            # объявлялась случайная пустая графа (порядок граф в выборке даже не
            # задан). На заполненную графу-итог условие не влияет.
            if x["value"] > 0 and rest > 0 and abs(x["value"] - rest) <= max(1.0, 0.005 * x["value"]):
                total_field, base = x["field"], rest
                break

        # Итог, посчитанный системой, — СТРАХОВКА на случай, когда графы-итога в
        # форме нет вовсе или она осталась незаполненной, а составляющие графы
        # заполнены. В нынешних данных РЦО такого нет ни разу (0 пар из 3 348),
        # поэтому правило обязано быть осторожным: молчание дешевле неверного
        # числа. И подставлять свою сумму ВМЕСТО числа формы нельзя никогда —
        # посчитанное уходит отдельным полем и всегда помечено как посчитанное.
        computed_total, computed_from = None, 0
        if homogeneous and total_field is None:
            filled = [x for x in sums if x["value"]]
            # Широкий допуск нужен ТОЛЬКО чтобы промолчать: если графа-итог в
            # форме есть, но расходится с суммой сильнее 0,5 %, арифметикой мы её
            # не опознали — а она уже сидит внутри `grand`, и наша «сумма»
            # удвоила бы итог. Замер по 54 выпускам РЦО на том же уровне, на
            # котором считает виджет (день, строки свёрнуты): «ИТОГО · Принято»
            # разошлось с суммой составляющих в 3 днях из 54 — 124 из 4 139
            # (3,0 %), 25 из 1 202 (2,1 %) и 69 из 5 517 (1,3 %). Порог 10 %
            # накрывает все три с троекратным запасом, а ошибка в эту сторону
            # стоит лишь молчания.
            looks_like_total = any(
                (grand - x["value"]) > 0
                and abs(x["value"] - (grand - x["value"])) <= 0.10 * x["value"]
                for x in filled)
            # «Итог» по одной графе — это сама графа, а не итог.
            if not looks_like_total and len(filled) >= 2 and grand > 0:
                computed_total, computed_from = grand, len(filled)
        ...
        return {"type": "field_list", "title": name, "rows": shown,
                ...
                # Посчитанное системой число идёт ОТДЕЛЬНЫМ полем и вместе с
                # числом граф, по которым сложено: по нему видно, что это не
                # цифра из формы. `total` выше — по-прежнему база доли, её
                # смысл не меняется.
                "computed_total": computed_total,
                "computed_total_fields": computed_from,
    ...
    if t != "pie":
        ...
        # и на полном наборе рисовал бы линию по строкам, которых на графике уже
        # нет.
