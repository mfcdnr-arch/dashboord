...
async def test_fingerprint_ignores_rolling_dates_in_headers():
    """🔴 Реальный случай (ДНР_статистика): графа называется «Количество
    принятых заявлений с 01.01.2026 по 19.08.2026», и через неделю то же самое
    поле — «…по 26.08.2026». Без вырезания даты отпечаток формы никогда не
    совпал бы с прошлым выпуском, и «Загрузка» просила бы ручную разметку
    КАЖДУЮ неделю — автораспознавание было бы бесполезно именно там, где
    нужнее всего (форма с ролящейся датой в заголовке)."""
    week1 = [["Субъект", "МФЦ (адрес)", "Количество принятых заявлений с 01.01.2026 по 19.08.2026"],
             ["ДНР", "Отделение №1", "100"]]
    week2 = [["Субъект", "МФЦ (адрес)", "Количество принятых заявлений с 01.01.2026 по 26.08.2026"],
             ["ДНР", "Отделение №1", "120"]]
    assert mapping.structure_fingerprint(week1, 1) == mapping.structure_fingerprint(week2, 1)
    # А настоящее переименование графы (не дата) отпечаток по-прежнему ловит.
    renamed = [["Субъект", "МФЦ (адрес)", "Количество выданных результатов с 01.01.2026 по 26.08.2026"],
               ["ДНР", "Отделение №1", "120"]]
    assert mapping.structure_fingerprint(week1, 1) != mapping.structure_fingerprint(renamed, 1)
