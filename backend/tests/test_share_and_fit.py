...
def test_explicit_count_unit_beats_the_word_guess():
    """🔴 Ложные срабатывания на длинных названиях госуслуг.

    Правило ищет приметы доли ВНУТРИ всего имени, и на форме РЦО это давало
    промахи: «…дошкольного, начального общего, основного общего и СРЕДНЕГО
    общего образования…» ловилось по «средн», а «Компенсация 50% ОСАГО
    инвалидам» — по «%». Обе графы — количества, обе кончаются на «, ед.»,
    и карточка по ним делила сумму на 62 отделения.

    Замер по всем 766 графам системы: правило срабатывает на 6, у 5 в имени
    стоит «, ед.», и лишь одна — настоящая доля.
    """
    осаго = "СФР · Компенсация 50% ОСАГО инвалидам (266) · Принято, ед."
    образование = ("Администрация м.о. Дебальцево · (428) Предоставление информации "
                   "об организации общедоступного и бесплатного дошкольного, начального "
                   "общего, основного общего и среднего общего образования · Принято, ед.")
    assert not is_share(осаго)
    assert not is_share(образование)
    # Считается суммой, а не средним: 62 отделения по 1 обращению — это 62.
    value, how = aggregate_series([1.0] * 62, осаго)
    assert (value, how) == (62.0, "sum")


def test_real_share_is_still_recognised():
    """Оговорка не должна отключить правило: единственная настоящая доля
    системы («% достижения показателя») по-прежнему усредняется."""
    assert is_share("% достижения показателя")
    assert is_share("Доля доставленных уведомлений · Факт · нарастающим итогом, %")
    assert is_share("Удельный вес обращений")
    # Явная единица, заданная человеком в виджете, тоже продолжает работать.
    assert is_share("Обращения", "%")


def test_frontend_copy_of_the_rule_matches():
    """Правило продублировано в конструкторе разметки (DashboardDraft.tsx).

    Расхождение означало бы, что один и тот же столбец даёт в конструкторе
    одно число, а на дашборде другое — ровно то, о чём предупреждает
    docstring самого модуля. Держим копию честной тестом.
    """
    from pathlib import Path

    src = (Path(__file__).resolve().parents[2]
           / "frontend/src/components/DashboardDraft.tsx").read_text(encoding="utf-8")
    assert "COUNT_UNIT_RE" in src, "во фронте нет оговорки про единицу количества"
    # Обе стороны должны отсекать по одному и тому же набору единиц.
    for unit in ("ед", "шт", "чел"):
        assert unit in src.split("COUNT_UNIT_RE")[1].split("\n")[0]
    # И порядок проверок тот же: единица отменяет догадку, а не наоборот.
    assert "!COUNT_UNIT_RE.test(name) && SHARE_RE.test(name)" in src
