...
def test_big_book_fits_into_the_job_timeout():
    """🔴 Большая книга должна успевать разобраться, а не сниматься по таймауту.

    Найдено загрузкой на боевой 22.09.2026: книга за год (21,6 МБ, 201 лист)
    разбирается дольше пяти минут — сам разбор занимает секунды, а вот запись
    201 таблицы с полной сеткой каждая идёт долго. Умолчание arq — 300 с, и
    задание снималось по таймауту. На экране это выглядело как «файл
    распознаётся» без конца: задание помечено failed, причина — только в логе
    контейнера.

    До этого дефект не проявлялся ни разу, потому что на дев-стенде такую книгу
    выпускали вызовом `run_extraction` напрямую, минуя очередь: путь, которым
    пойдёт ЗАКАЗЧИК (положить файл в «Загрузку»), на большой книге не
    проверялся вовсе.
    """
    from app.modules.ingestion.worker import WorkerSettings

    timeout = getattr(WorkerSettings, "job_timeout", 300)
    assert timeout >= 1800, (
        f"job_timeout={timeout} с: книга за год не успеет разобраться, и задание "
        "снимут по таймауту — молча, без следа на экране")
