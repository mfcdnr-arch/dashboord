...
async def test_delete_draft_metric_removes_versions_and_audits(client, admin_headers):
    ...
    try:
        ...
        r = await client.delete(f"/metrics/{mid}", headers=admin_headers)
...
async def test_delete_blocked_while_widget_uses_metric(client, admin_headers):
    ...
    try:
        ...
        assert (await client.delete(f"/metrics/{mid}", headers=admin_headers)).status_code == 200
    finally:
        if did:
...
async def test_delete_blocked_while_other_formula_references_it(client, admin_headers):
...
async def test_delete_requires_manage_rights(client, admin_headers, viewer):
    code = "ztest_del_rights"
    try:
        mid = await _metric_with_version(client, admin_headers, code)
        assert (await client.delete(f"/metrics/{mid}", headers=viewer["headers"])).status_code == 403
    finally:
        await _purge([code])


async def test_delete_missing_metric_gives_404(client, admin_headers):
    import uuid
    r = await client.delete(f"/metrics/{uuid.uuid4()}", headers=admin_headers)
