...
type Overview = {
  ...
  /** Есть ли в разделе данные вообще и по скольким ведомствам. */
  ready?: boolean
...
function OverviewView({ onOpenList }: { onOpenList: () => void }) {
  ...
  return (
    <div>
      ...
      {/* 🔴 Раздел без данных печатал стену нулей и «Замечаний нет» — это
          читается как «система сломалась», а не как «файлов ещё не было».
          Ровно так и вышло на боевом 22.09.2026, где ведомственных файлов нет
          вовсе. Пустота обязана объяснить себя и сказать, откуда берутся
          данные: рядовому пользователю пункт меню вообще не показывается, а
          управляющий должен понимать, что делать. */}
      {d && d.ready === false && (
        <div style={{ ...panelStyle, marginTop: 12 }}>
          <h3 style={panelTitle}>Данных пока нет</h3>
          <div style={{ color: 'var(--text-muted)', fontSize: 14, lineHeight: 1.6 }}>
            Раздел собирается из еженедельных файлов «ДНР_статистика_на_ДД.ММ.ГГГГ.xlsx»:
            в такой книге по листу на каждое ведомство, а внутри листа — строки отделений
            и блок столбцов на каждую услугу.
            <br />
            Размечено ведомств: <b>{d.departments_with_data ?? 0}</b> из {d.departments_total ?? 12}.
            <br /><br />
            Чтобы раздел заработал, нужно завести объект ведомства в разделе «Объекты»
            (по объекту на ведомство — у каждого свой бланк) и загрузить файл в разделе
            «📥 Загрузка»: знакомую форму система узнаёт сама.
            <br /><br />
            Пока ни одного ведомства не размечено, показывать здесь нечего — цифры ниже
            были бы нулями, а не данными.
          </div>
        </div>
      )}
