        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap', marginTop: 2 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            <Dropdown label="⋯" title="Остальные действия с дашбордом" open={moreOpen} setOpen={setMoreOpen}>
              {canManage && (
                /* 🔴 Пункт называет ВСЁ, что включает, а не только перетаскивание.
                   Жалоба заказчика 22.09.2026: «хотел поменять размер и вид
                   виджета — не получилось». Всё работало, но кнопки ✎ и ручка
                   размера появляются только в режиме правки, а он спрятан здесь
                   и обещал лишь «двигать виджеты» — искать в нём смену вида
                   никто не станет.
                   В «потоке» размер и место считаются по типу виджета, поэтому
                   там обещать размер нельзя: подпись честно говорит, что даёт
                   правка, и куда идти за размером. */
                <button type="button" style={{ ...menuItem, color: editMode ? 'var(--accent-text)' : 'var(--text-2)' }}
                  title={flowMode
                    // 🔴 Кнопка ⧉ сама живёт в режиме правки, поэтому подсказка
                    // ведёт по шагам: сначала включить правку, потом она появится.
                    // Иначе получается замкнутый круг — «нажмите кнопку, которой
                    // не видно».
                    ? 'В «потоке» размер и место виджетов система считает сама. Включите правку — у виджетов появится ✎ (изменить данные и вид), а в шапке кнопка ⧉, которая переводит страницу на «Сетку», где размер тянут мышью.'
                    : 'Появятся ✎ (изменить данные и вид), 🗑 и ручка размера в правом нижнем углу карточки.'}
                  onClick={() => { setEditMode(!editMode); setMoreOpen(false) }}>
                  {editMode
                    ? '✓ Выйти из режима правки'
                    : flowMode ? '✎ Правка: менять вид виджетов' : '✎ Правка: двигать, менять размер и вид'}
                </button>
              )}
