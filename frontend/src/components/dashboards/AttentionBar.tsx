    ...
    <div data-export-hide style={{
      ...
      {open && (
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {/* Подпись обязана совпадать с тем, что показано выше. До п. 13 здесь
              стояло «сверка последнего отчёта с предыдущим» — с появлением
              проверок ВНУТРИ файла это стало неправдой, и на первом отчёте
              формы (сравнивать не с чем) текст прямо противоречил бы списку
              замечаний над ним. */}
          <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>
            Это те же проверки, что видит модератор при выпуске данных: арифметика внутри отчёта
            {items.some((it) => it.previous_period) ? ' и сверка с предыдущим отчётом' : ''}.
            Замечание не значит, что цифры неверны, — значит, что их стоит проверить.
          </div>
        </div>
      )}
    </div>
