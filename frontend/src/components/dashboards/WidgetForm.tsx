    ...
    <form onSubmit={submit} style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'flex-end', minWidth: 0, maxWidth: '100%',
      ...
      {type === 'status_grid' && (
        <F t="Поле плана (необязательно) — тогда цвет по % выполнения">
          ...
          {/* Светофор без порогов красит все плитки одинаково — то есть выглядит
              сломанным ровно в том, ради чего его берут. С планом норма известна
              (100 % — это сам план), поэтому она подставляется сразу; без плана
              нормы нет, и выдуманное правило было бы хуже отсутствующего. */}
          <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 4, maxWidth: 320 }}>
            {planField
