    ...
    return (
      <div style={{ fontSize: 13, overflow: 'auto', height: '100%' }}>
        ...
        <div style={{ ...muted, marginTop: 6 }}>
          ...
          {/* Прирост без даты, с которой он считан, — это половина ответа:
              «+95» к чему? Называем отчёт прямо. */}
          {data.prev_period && ` · изменение к отчёту за ${fmtPeriod(data.prev_period)}`}
          {data.has_total_row && ' · графа-итог найдена по арифметике и в доле не участвует'}
          {/* Посчитанный системой итог обязан быть отличим от числа формы, иначе
              человек примет нашу сумму за цифру отчёта. Живёт в подвале рядом с
              остальными оговорками, а не строкой таблицы: сортировка и
              группировка её не тронут, и в счётчик «показателей: N» она не
              попадёт. Формулировка говорит «среди показанных не нашлось», а не
              «в форме такой графы нет»: мы знаем только про показанные графы —
              человек мог выбрать часть списка, и утверждать что-то про всю
              форму мы не вправе. */}
          {data.computed_total != null && (
            <div style={{ marginTop: 2 }}>
              Σ итог посчитан системой по {data.computed_total_fields}{' '}
              {plural(data.computed_total_fields, 'графе', 'графам', 'графам')}:{' '}
              <b>{fmt(data.computed_total)}</b> — графы-итога среди показанных не нашлось
            </div>
          )}
  ...
  if (data.type === 'compare' || data.type === 'cross_dataset_compare') {
    ...
    const opt: EChartsOption = {
      grid: { left: gridLeft((data.series || []).flatMap((x: any) => x.data || [])), right: 12, top: 12,
        bottom: cmpBottom },
      // На логарифме над каждым столбиком подписано точное число (длина
      // столбика обманчива), а при многих сериях в узкой группе подписи
      // соседних столбиков ложатся друг на друга. Ширины (только что
      // отведённой во всю строку) может не хватить и на широком мониторе —
      // `hideOverlap` прячет лишние подписи вместо того, чтобы дать им слипнуться.
