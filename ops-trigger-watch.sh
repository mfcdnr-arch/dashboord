...
# 🔴 Сторож фонового воркера — ПЕРЕД выходом по отсутствию заявки на бэкап.
# Иначе он не запускался бы никогда: строка ниже завершает скрипт в подавляющем
# большинстве запусков. Воркер тихо останавливает весь конвейер, а попросить о
# собственном перезапуске не может — поднять его способен только хост.
# `|| true`: сбой сторожа не должен мешать бэкапу — это разные обязанности.
[ -x "$REPO/worker-guard.sh" ] && "$REPO/worker-guard.sh" || true

# Сторож доступности — по той же причине и в том же месте: единственный сигнал,
# уходящий НАРУЖУ (системный журнал + файл состояния). Сам решает, пора ли
# спрашивать /health (по умолчанию раз в 5 минут), поэтому зовём его каждый раз.
[ -x "$REPO/health-watch.sh" ] && "$REPO/health-watch.sh" || true
