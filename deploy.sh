...
# Пауза сторожа доступности на время обновления. Система на эти минуты и правда
# недоступна, но дежурному сообщать нечего: это плановая работа. Без паузы
# КАЖДЫЙ деплой поднимал бы тревогу, и сигнал перестали бы читать — то есть он
# не сработал бы в настоящий раз. Снимается при ЛЮБОМ выходе, включая ошибку:
# иначе неудачный деплой оставил бы сторожа выключенным навсегда, и падение
# после него осталось бы незамеченным.
HEALTH_OFF="$(pwd)/ops-triggers/health-watch.off"
mkdir -p "$(dirname "$HEALTH_OFF")" 2>/dev/null || true
: > "$HEALTH_OFF" 2>/dev/null || true
trap 'rm -f "$HEALTH_OFF" 2>/dev/null || true' EXIT
